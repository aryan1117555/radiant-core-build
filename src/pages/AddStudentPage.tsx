...
const AddStudentPage = () => {
  ...
  return <>
      ...
      <div className="rounded-lg shadow p-6 mb-6 bg-slate-950">
        <div className="mb-6">
          <h2 className="text-lg font-medium mb-4">Select Room</h2>
          {availableRooms.length > 0 ? <div>
              <select value={selectedRoomId} onChange={handleRoomChange} disabled={saving} className="w-full rounded-md border border-input p-3 text-sm bg-slate-950">
              ...
              {selectedRoomId && <div className="mt-3 p-3 bg-blue-50 rounded-md">
                  {(() => {
              const selectedRoom = rooms.find(r => r.id === selectedRoomId);
              return selectedRoom ? <div className="text-sm text-blue-800">
            ...
            </div> : <div className="p-4 bg-yellow-50 text-yellow-800 rounded-md">
              <p className="font-medium">No rooms available</p>
              <p className="text-sm mt-1">
                {rooms.length === 0 ? "No rooms found. Please create some rooms first." : "All rooms are at full capacity."}
              </p>
              {rooms.length === 0 && <Button variant="outline" size="sm" className="mt-2" onClick={() => navigate('/room-management')}>
                  Go to Room Management
                </Button>}
            </div>}
        </div>
        
        {selectedRoomId && <AddStudentForm roomId={selectedRoomId} onAddStudent={handleAddStudent} selectedRoom={rooms.find(room => room.id === selectedRoomId)} disabled={saving} student={isEditMode ? student : undefined} />}
      </div>
    </>;
};
